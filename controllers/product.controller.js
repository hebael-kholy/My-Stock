...
class productController {
    createProduct = asyncHandler(async(req, res, next) =>{
        const {title,description, price, brand,isSale,quantity,category,rating} = req.body;
    ...
    uploadImage = asyncHandler(async(req,res,next)=>{
        ...
        const product = await Product.findOneAndUpdate({id},{image:result.url},{new:true});
    ...
    findAll = asyncHandler(async(req,res,next)=>{
        ...
        const product = await Product.find(filter).skip(skip).limit(limit).populate({path:"category",select:"name"});
        if (!product){ return next(new ApiError(`Can't find Products`, 404));}
        res.status(200).json({
    ...
    findone = asyncHandler(async (req, res,next) => {
        const {id} = req.params;
        const product = await Product.findOne({id}).populate({path:"category",select:"name"});
    ...
    updateOne = asyncHandler(async(req,res,next)=>{
        ...
        const user = await Product.findByIdAndUpdate(id,{title,description, price, brand,isSale,quantity,category,rating},{new:true});
    ...
    deleteOne = asyncHandler(async(req,res,next)=>{
        ...
        if (!product){ return next(new ApiError(`Invalid id ${id} `, 404));}
